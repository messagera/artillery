...
function run(scriptPath, options) {
  ...
  app.post('/receiveMessage', function(req, res){
    ...
    assistants[req.body.from].responses = assistants[req.body.from].responses + 1 || 1;
    userIds[req.body.to.id].responses = userIds[req.body.to.id].responses + 1 || 1;
...
function humanize(report) {
  ...
  fs.writeFile('assistantLog.txt', `${JSON.stringify(assistants)}\n`, (err) => {
      if (err) throw err;
    });
  fs.writeFile('userLog.txt', `${JSON.stringify(userIds)}\n`, (err) => {
      if (err) throw err;
    });
  fs.writeFile('timestamps.json', `${JSON.stringify(timestamps)}\n`, (err) => {
      if (err) throw err;
    });
}
